...
class Command(BaseCommand):
    ...
    def handle(self, *args, **opts):
        ...
        # Scale full dataset BUT keep column names by wrapping back into a DataFrame
        ...
        # Train final model ON A DATAFRAME to retain feature_names_in_
        if hasattr(best_model, 'fit'):
            if isinstance(best_model, HistGradientBoostingClassifier):
                sample_weights = compute_sample_weights(y)
                best_model.fit(X_full_scaled_df, y, sample_weight=sample_weights)
            else:
                smote = SMOTE(random_state=42, k_neighbors=5)
                X_resampled, y_resampled = smote.fit_resample(X_full_scaled_df, y)
        ...
        # Save datasets
        train_csv = os.path.join(export_dir, "dot_train_dataset.csv")
        test_csv = os.path.join(export_dir, "dot_test_dataset.csv")
        preds_csv = os.path.join(export_dir, "dot_predictions.csv")
        ...
        model_path = os.path.join(export_dir, "dot_long_hgb_model.joblib")
        scaler_path = os.path.join(export_dir, "dot_feature_scaler.joblib")
        features_path = os.path.join(export_dir, "dot_feature_list.json")
        config_path = os.path.join(export_dir, "dot_trade_config.json")
