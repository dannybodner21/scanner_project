...
class Command(BaseCommand):
    help = "Backfill all missing indicators from April 23 to May 2, 2025."

    def handle(self, *args, **kwargs):
        start = make_aware(datetime(2025, 4, 23))
        end = make_aware(datetime(2025, 5, 2)) + timedelta(days=1)

        # Only get entries with at least one missing field
        qs = RickisMetrics.objects.filter(
            ...
            Q(stochastic_d__isnull=True) |
            Q(support_level__isnull=True) |
            Q(resistance_level__isnull=True) |
            Q(relative_volume__isnull=True) |
            Q(sma_5__isnull=True) |
            Q(sma_20__isnull=True) |
            Q(stddev_1h__isnull=True) |
            Q(atr_1h__isnull=True) |
            Q(obv__isnull=True)
        ...
        print(f"🔍 Processing {total} RickisMetrics entries with missing indicators...")
