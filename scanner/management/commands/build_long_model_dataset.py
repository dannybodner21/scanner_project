...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        # Label generation with TP=6%, SL=3%, 24h window (288 5min intervals)
        full_df = self.generate_labels(full_df, tp=0.06, sl=0.03, window=288)
        self.stdout.write("Labels generated.")

        # Balance dataset by sampling equal wins/losses
        balanced_df = self.balance_data(full_df)
        self.stdout.write(f"Balanced dataset size: {len(balanced_df)}")

        # Split out 2025+ data for testing
    ...
    def add_features(self, df):
        ...
        df['obv_slope'] = df['obv'].diff()  # simple slope approx
        ...
        # Market regime based on ADX and price relative to MA200
    ...
    def balance_data(self, df):
        ...
        # Randomly sample losses to match wins count
