# scanner/management/commands/calculate_market_regime_live.py
# Live market regime calculator - processes every 5-minute candle
# Uses 1-hour lookback window (12 bars) for regime detection
...
class Command(BaseCommand):
    help = 'Calculate market regime for every 5-minute candle using 4-hour lookback window'

    def add_arguments(self, parser):
        parser.add_argument('--output-file', type=str, default='market_regime_live.csv', help='Output CSV file')
        parser.add_argument('--lookback-bars', type=int, default=96, help='Bars to look back for regime detection (96 = 8 hours)')
        parser.add_argument('--regime-threshold', type=float, default=0.4, help='Min difference to switch regimes')
        parser.add_argument('--bias-correction', type=str, default='equal_weight', 
    ...
    def calculate_regime_live(self, predictions, all_timestamps, lookback_bars=12, bias_correction='equal_weight'):
        """
        Calculate regime for every 5-minute timestamp using 1-hour lookback window
        This is the live version that processes every single timestamp
        """
        ...
        # Apply rolling window for lookback (1-hour window = 12 bars)
    ...
    def handle(self, *args, **opt):
        ...
        self.stdout.write("🚀 LIVE MARKET REGIME CALCULATOR")
        self.stdout.write("📊 Processing every 5-minute candle with 1-hour lookback window")
        self.stdout.write(f"🔧 Bias correction method: {bias_correction}")
        self.stdout.write(f"⏰ Lookback window: {lookback_bars} bars ({lookback_bars * 5} minutes)")
        ...
        self.stdout.write(f"▶ Processing {len(all_timestamps)} timestamps (every 5-minute candle)...")
        self.stdout.write(f"📅 Date range: {min(all_timestamps)} to {max(all_timestamps)}")
        
        # Calculate regime for all timestamps at once (FAST!)
        regime_df = self.calculate_regime_live(predictions, all_timestamps, lookback_bars, bias_correction)
        ...
        self.stdout.write(self.style.SUCCESS(f"\n✅ LIVE MARKET REGIME CALCULATION COMPLETE"))
        ...
        # Show regime change frequency
        regime_changes = (regime_df['regime'] != regime_df['regime'].shift()).sum() - 1  # -1 for first row
        self.stdout.write(self.style.SUCCESS(f"🔄 Total regime changes: {regime_changes}"))
        self.stdout.write(self.style.SUCCESS(f"📊 Average regime duration: {total_timestamps / regime_changes:.1f} timestamps ({total_timestamps / regime_changes * 5:.1f} minutes)"))
